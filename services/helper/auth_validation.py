...
async def password_check_validation(password, user):
    try:
        password_check = await pwd_context.verify(password, user.password)
...
async def user_check(data:dict):
    try:
        ...
        user = await repo.find_user_by_field(field_name=field_name, field_value=data[field_name])
        if not user:
